...
VALID_INTERVALS = {"1m","2m","5m","15m","30m","1h","1d"} # "60m","90m","5d","1wk","1mo","3mo"
# Enable "4h" if doing forex in the future 
...
def get_historical_data(ticker:str, 
    ...
    if interval == "1d":
        ...
        if interval == '1h': 
            # Keep YF's timestamps which are on the hour pre- and post- market, and on 30th minutes during market hours. Fill in only the pre- and post- market volume from Polygon. This ensures a separate candlestigit ck when market opens at 9:30am. 
            
            pre_post_data = new_data_yf.loc[(new_data_yf['Datetime'].dt.time <= MARKET_OPEN) | (new_data_yf['Datetime'].dt.time >= MARKET_CLOSE)]
            # TODO: paste code from previous commit to fill in volume here and produce new_data
